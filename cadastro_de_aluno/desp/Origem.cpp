...
void cadastro() // implementa a funcao cadastro
{
	...
	while (1) {
		...
		scanf_s("%s", &cad_atual->nome, 60); //Nome tem no m�ximo 60 caracteres
		while ((c = getchar()) != '\n' && c != EOF) {}
		printf("Digite o email:");
		scanf_s("%s", &cad_atual->email, 100); //Email tem no m�ximo 100 caracteres
...
void pesquisa() // implementa a funcao pesquisa
{
	...
	do {
		...
		case 2:
			...
			scanf_s("%s", nome_aux, 60);//n�o precisa de & para string
			while ((c = getchar()) != '\n' && c != EOF) {}
			while (prox_cad != NULL) {
